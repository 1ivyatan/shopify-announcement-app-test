...
type SupportedLocale = (typeof SUPPORTED_APP_LOCALES)[number];
...
let _userLocale: SupportedLocale;
...
export function getUserLocale(): SupportedLocale {
  ...
  _userLocale = match([locale], SUPPORTED_APP_LOCALES, DEFAULT_APP_LOCALE) as SupportedLocale;
  return _userLocale;
}
...
const PLURAL_RULES_LOCALE_DATA: Record<string, () => Promise<any>> = {
  ...
  no: () => import("@formatjs/intl-pluralrules/locale-data/nb"),
  ...
  "zh-CN": () => import("@formatjs/intl-pluralrules/locale-data/zh"),
};

async function loadIntlPluralRulesLocaleData(locale: string): Promise<any> {
  if (!PLURAL_RULES_LOCALE_DATA[locale]) {
    console.warn(`No plural rules locale data found for ${locale}, falling back to default locale`);
    return (await PLURAL_RULES_LOCALE_DATA[DEFAULT_APP_LOCALE]()).default;
  }
...
const POLARIS_LOCALE_DATA: Record<string, () => Promise<any>> = {
  ...
  no: () => import("@shopify/polaris/locales/nb.json"),
